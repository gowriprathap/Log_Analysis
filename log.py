...
query1 = """SELECT title, count(*) AS views FROM articles JOIN log
         ON articles.slug = substring(log.path,10)
         GROUP BY title ORDER BY views DESC LIMIT 3;"""   # First query
...
query2 = """SELECT authors.name, count(*) AS views FROM authors
         JOIN articles ON articles.author = authors.id JOIN log
         ON articles.slug = substring(log.path,10) GROUP BY authors.name
         ORDER BY views DESC;"""  # Second query
...
query3 = """SELECT to_char(day2, 'Mon DD, YYYY'),
         round((numstats*100.0)/visitors, 3)
         AS result FROM errorsum ORDER BY result DESC LIMIT 1"""  # Third query
...
def print_query(query_results):  # Printing first and second queries
    ...
    for result in query_results['results']:
        ...
        # Printing the first two parts of the result
...
# Getting the query and storing in the dict object
